...
@testset "AlgebraicDynamics.jl integration: DWD test" begin
    ...
    # Compose
    rabbitfox_system_wrap = ⊕(
        rabbit_wrap, fox_wrap; diagram = rabbitfox_pattern,
    ...
    @testset "AlgebraicDynamics.jl and (wrapped) AlgebraicAgents.jl solutions are equal" begin
        ...
        @test isapprox(sol_ode.u, sol_agents.u, rtol = 1.0e-2)
    end
end
...
@testset "AlgebraicDynamics.jl integration: UWD test" begin
    ...
    rabbitfox_system = oapply(
        rabbitfox_pattern,
        [rabbit_growth, rabbitfox_predation, fox_decline]
    )

    rabbit_growth_wrap = wrap_system(
        "rabbit_growth",
        ContinuousResourceSharer{Float64}(1, dotr)
    )
    rabbitfox_predation_wrap = wrap_system(
        "rabbitfox_predation",
    ...
    # Compose
    rabbitfox_system_wrap = ⊕(
        rabbit_growth_wrap, rabbitfox_predation_wrap,
