...
prob_2 = SDEProblem(f, g, 1.2, tspan, [0.005, 0.02])

demand_model_1 = DiffEqAgent("demand", prob_1, EM(); exposed_ports = Dict("demand" => 1),
                             dt)
demand_model_2 = DiffEqAgent("demand", prob_2, EM(); exposed_ports = Dict("demand" => 1),
                             dt)
...
# set parameters
@test getparameters(pharma_model)["therapeutic_area1/demand/"] == [0.01, 0.01]
setparameters!(pharma_model, Dict("therapeutic_area1/demand/" => [0.02, 0.02]))
@test getparameters(pharma_model)["therapeutic_area1/demand/"] == [0.02, 0.02]
...
i = 5;
q1 = pharma_model |> @filter(_.age<$i) |> @filter('m' ∈ _.name);
...
o = objective(pharma_model, 100.0)

## get results given params
o(Dict("therapeutic_area1/demand" => [0.02, 0.02]))
...
@time o_avg(Dict("therapeutic_area1/demand" => [0.02, 0.02]))
